<?php
    ...
    if (isset($_REQUEST['connexion']))
    {
        ...
        $register = $arrondissement->doInsertarrondissement($nom, $commune, $genre, $residence, $contact, $fonction, $datedebut, $datefin, $datecreation, $status);
        
        if ($register) 
        {
            $user->redirect('succes.php?q=arrondissement');
        
        } else 
        {
            
            $user->redirect('echec.php?q=arrondissement');
...
<div id="wrap" class="">
	...
	<div id="page-body" class="page-width container ">
		...
		<?php
			...
			{
				$updateId = $_GET['id'];
				$myarrondissementIs = $arrondissement->getarrondissementById($updateId);
				$theAllcommune = $communeObj->getAllcommune();
				
				
				if(count($myarrondissementIs) > 0)
				{
					...
					if (isset($_REQUEST['modifier']) AND isset($_GET['q']) && $_GET['q']=='u')
					{
						...
						if(!empty($nom) AND $commune != '' AND !empty($genre) AND !empty($residence) AND !empty($contact) AND !empty($fonction) AND !empty($datedebut) AND !empty($datefin) AND $status != '')
						{
							$datecreation = date("Y-m-d H:i:s");				
							//$update = $arrondissement->doUpdatearrondissementById($nom, $commune, $genre, $residence, $contact, $fonction, $datedebut, $datefin, $datecreation, $status, $updateId);
							
							if($arrondissement->doUpdatearrondissementById($nom, $commune, $genre, $residence, $contact, $fonction, $datedebut, $datefin, $datecreation, $status, $updateId)) 
							{
								$user->redirect('succes.php?q=arrondissement');
					...
					echo'
						...
						<div class="action-bar bottom topic-actions view-topic  ">
								...
								<div class="float-right">
									...
									<div class="btn btn-default navbar-btn">
										<a href="/arrondissement.php" >Retour a la liste</a>
									</div>
						...
						<div class="row">
							<div class="col-lg-12">
								<div class="panel panel-default">
									<div class="panel-heading"><h3 style="color:white;">Modifier '.utf8_encode($myarrondissementIs['name']).'</h3> </div>
										...
										<div class="col-md-6 col-md-6-form" style="margin-left:10%;">
											<form role="form" method="post" action="">
												...
												<div class="form-group">
													<label>Nom</label>
													<input type="text" name="nom" class="form-control" value="'.utf8_encode($myarrondissementIs['name']).'">
												</div>
												...
												<select class="form-control" name="commune">';
													...
													foreach ($theAllcommune as $key => $value)
													{
												...
												<div class="form-group">
													...
													<input type="text" name="residence" class="form-control" value="'.utf8_encode($myarrondissementIs['residence']).'">
												</div>
												
												<div class="form-group">
													<label>Fonction Exercee</label>
													<input type="text" name="fonction" class="form-control" value="'.utf8_encode($myarrondissementIs['fonction']).'">
												</div>
												
												<div class="form-group">
													<label>Contact</label>
													<input type="text" name="contact" class="form-control" value="'.utf8_encode($myarrondissementIs['contact']).'">
												</div>
												
												<div class="form-group">
													<label>Date Debut mandat</label>
													<input class="datepicker form-control" name="datedebut"  data-date-format="mm/dd/yyyy" value="'.utf8_encode($myarrondissementIs['dateDebut']).'">
												</div>
												
												<div class="form-group">
													<label>Date Fin mandat</label>
													<input class="datepicker form-control" name="datefin"  data-date-format="mm/dd/yyyy" value="'.utf8_encode($myarrondissementIs['dateFin']).'">
												</div>
			...
			elseif (isset($_GET['q']) && $_GET['q']=='d')
			{
				...
				$myarrondissementIs = $arrondissement->getarrondissementById($deleteId);
				
				if (isset($_REQUEST['supprimer']) AND isset($_GET['q']) && $_GET['q']=='d')
				{
					extract($_REQUEST);
					
					$status = (isSet($myarrondissementIs['status']) AND  $myarrondissementIs['status'] == 0)? 1 : 0;
					
					$delete = $arrondissement->doDeletearrondissementById($status, $deleteId);
				...
				if($delBool)
				{
					echo'<center><div class="btn btn-default navbar-btn"><a href="/arrondissement.php" >Retour a la liste</a></div></center>';
				}
				else
				{
					echo '
						...
						<div class="alert bg-warning" role="alert">
							 Confirmer cette op&eacute;ration ? ('.$myarrondissementIs['fullname'].' ) 
						</div>
						<center>
						<form role="form" method="post" action="">
							
							<div class="btn btn-default navbar-btn"><a href="/arrondissement.php" >Annuler</a></div>
			...
			elseif (isset($_GET['q']) && $_GET['q']=='add')
			{
				...
				$thearrondissement = $arrondissement->getAllarrondissement();
				$theAllcommune = $communeObj->getAllcommune();
				...
				echo'
					...
					<div class="action-bar bottom topic-actions view-topic  ">
							...
							<div class="float-left">
								...
								<!--<div class="btn btn-default navbar-btn">
									<a href="/arrondissement.php?q=add" class="">Ajouter</a>
								</div>-->
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="panel panel-default">
								<div class="panel-heading"><h3 style="color:white;">Ajouter un nouveau arrondissement</h3> </div>
								<div class="panel-body">
									<div class="col-md-6 col-md-6-form" style="margin-left:10%;">
										<form role="form" method="post" action="">
				...
				echo '
					...
					<div class="action-bar bottom topic-actions view-topic  ">
						...
						<div class="float-left">
							...
							<div class="btn btn-default navbar-btn">
								<a href=""> Total : &nbsp;&nbsp;<span class="badge">'.$arrondissement->doarrondissementCount().'</span></a>
							</div>
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="panel panel-default">
								<div class="panel-heading"><h3 style="color:white;">Liste des arrondissements</h3> </div>
								<div class="panel-body">
									<!--<table class="table table-bordred table-striped" data-toggle="table" data-url="adminbaba/tables/arrondissementtable.json"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="asc">-->
									<table class="table table-bordred table-striped" data-toggle="table" data-url="table.arrondissement.php"  data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-search="true" data-select-item-name="toolbar1" data-pagination="true" data-sort-name="name" data-sort-order="asc">
